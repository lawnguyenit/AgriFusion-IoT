...
from ..processors.context import extract_raw_buffer_reason
...
from ..writers import CanonicalOutputWriter, DebugViewWriter, SupplementalOutputWriter
...
class PreprocessingPipeline:
    def __init__(
        ...
        *,
        ...
        self._supplemental_writer = SupplementalOutputWriter(self.output_root)
    ...
    def run(self) -> Layer1Result:
        ...
        buffer_reason_audit_rows: list[dict[str, object]] = []

        for source_record in source_records:
            row = self._row_builder.build(source_record)
            raw_buffer_reason = extract_raw_buffer_reason(source_record)
            if raw_buffer_reason is not None:
                buffer_reason_audit_rows.append(
                    {
                        "record.id": row["record.id"],
                        "record.segment_id": None,
                        "record.node_id": row["record.node_id"],
                        "delivery.buffer_reason": row["delivery.buffer_reason"],
                        "buffer_reason_raw": raw_buffer_reason,
                    }
                )
        ...
        canonical_df, duplicate_count, segment_summaries = apply_temporal_features(
            canonical_df,
            self.temporal,
        )
        stats.duplicate_record_id_count = duplicate_count
        stats.segment_count = len(segment_summaries)
        excluded_df = pd.DataFrame(excluded_rows)
        buffer_reason_audit_df = pd.DataFrame(buffer_reason_audit_rows)
        if not buffer_reason_audit_df.empty and not canonical_df.empty:
            segment_map = canonical_df.loc[
                :,
                ["record.id", "record.segment_id"],
            ].drop_duplicates()
            buffer_reason_audit_df = buffer_reason_audit_df.merge(
                segment_map,
                on="record.id",
                how="left",
                suffixes=("", "_derived"),
            )
            if "record.segment_id_derived" in buffer_reason_audit_df.columns:
                buffer_reason_audit_df["record.segment_id"] = buffer_reason_audit_df[
                    "record.segment_id_derived"
                ]
                buffer_reason_audit_df = buffer_reason_audit_df.drop(
                    columns=["record.segment_id_derived"]
                )
        stats.buffer_reason_audit_row_count = int(len(buffer_reason_audit_df))
        ...
        output_paths = self._write_outputs(
            ...
            buffer_reason_audit_df=buffer_reason_audit_df,
            stats=stats,
    ...
    def _write_outputs(
        ...
        *,
        ...
        buffer_reason_audit_df: pd.DataFrame,
        ...
        self._supplemental_writer.ensure_directories()
        ...
        buffer_reason_audit_path = self._supplemental_writer.write_buffer_reason_audit(
            buffer_reason_audit_df
        )
        segment_manifest_path = self._supplemental_writer.write_segment_outputs(
            canonical_df=canonical_df,
            segment_summaries=segment_summaries,
            export_debug_views=self.export_debug_views,
        )
        ...
        processing_report_path = self._report_writer.write_processing_report(
            ...
            unknown_entries=unknown_entries,
            segment_summaries=segment_summaries,
            buffer_reason_audit_path=buffer_reason_audit_path,
