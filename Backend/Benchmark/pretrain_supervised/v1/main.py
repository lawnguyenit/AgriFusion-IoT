...
from Backend.Benchmark.pretrain_supervised.v1.src.pipeline.train_pipeline import run_v1_pipeline


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train downstream classifiers on top of embedding-pretrain outputs.")
    parser.add_argument(
        "--event-csv",
        type=Path,
        default=None,
        help="Path to the real labeled CSV used for downstream label merge. Defaults to benchmark_input_labeled.csv.",
    )
    parser.add_argument(
        "--pretrain-checkpoint",
        type=Path,
        default=None,
        help="Path to a pretrain checkpoint. If omitted, the latest checkpoint is discovered automatically.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=None,
        help="Optional output root. Defaults to Backend/Benchmark/pretrain_supervised/v1/outputs.",
    )
    parser.add_argument(
        "--label-mode",
        choices=("auto", "binary", "ternary"),
        default="auto",
        help="Choose the supervised label scheme.",
    )
    parser.add_argument(
        "--min-class-support",
        type=int,
...
def main() -> None:
    ...
    report = run_v1_pipeline(config)
    print(f"Benchmark family: {report['benchmark_family']}")
    print(f"Benchmark version: {report['benchmark_version']}")
    print(f"Selected label mode: {report['selected_label_mode']}")
    print(f"Best model: {report['best_model_name']}")
    print(f"Best validation macro_f1: {report['best_validation_macro_f1']:.4f}")
