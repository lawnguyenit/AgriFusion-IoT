...
def _slice_features(features: np.ndarray, split_slices: dict[str, slice]) -> dict[str, np.ndarray]:
    return {name: features[slice_obj] for name, slice_obj in split_slices.items()}
...
def run_v1_pipeline(config: V1Config) -> dict[str, object]:
    ...
    metrics_frame = pd.DataFrame(model_metrics_rows).sort_values(
    ...
    write_json(run_config_path, config.to_dict())
    ...
    return {
        "benchmark_family": config.benchmark_family,
        "benchmark_version": config.benchmark_version,
        "selected_label_mode": label_policy.label_mode,
        "best_model_name": best_result.model_name,
        "best_validation_macro_f1": best_validation_macro_f1,
        "output_dir": str(output_dir),
