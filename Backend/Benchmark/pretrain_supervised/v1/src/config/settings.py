...
class V1Config:
    ...
    def validate(self) -> None:
        ...
        if self.pretrain_checkpoint is not None and not self.pretrain_checkpoint.exists():
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        payload["pretrain_checkpoint"] = str(self.pretrain_checkpoint) if self.pretrain_checkpoint is not None else None
        payload["output_root"] = str(self.output_root)
        payload["default_pretrain_output_roots"] = [str(path) for path in DEFAULT_PRETRAIN_OUTPUT_ROOTS]
        return payload
