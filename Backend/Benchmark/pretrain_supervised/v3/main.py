...
from Backend.Benchmark.pretrain_supervised.v3.src.config.settings import V3Config
from Backend.Benchmark.pretrain_supervised.v3.src.pipeline.train_pipeline import run_v3_pipeline


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train downstream v3 models on embeddings produced from the multi-window combo benchmark."
    )
    parser.add_argument(
        "--event-csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=None,
        help="Subset of multi-window combo experiments to benchmark. Defaults to all combos.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=None,
        help="Optional output root. Defaults to Backend/Benchmark/pretrain_supervised/v3/outputs.",
    )
    parser.add_argument(
        "--label-mode",
