...
from Backend.Benchmark.pretrain_supervised.v4.src.config.settings import V4Config
from Backend.Benchmark.pretrain_supervised.v4.src.pipeline.train_pipeline import run_v4_pipeline


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train downstream v4 models on embeddings produced from the full single-window feature export."
    )
    parser.add_argument(
        "--event-csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        choices=("exp6",),
        default=None,
        help="Single-window full-set experiment to benchmark. Defaults to exp6.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=None,
        help="Optional output root. Defaults to Backend/Benchmark/pretrain_supervised/v4/outputs.",
    )
    parser.add_argument(
        "--label-mode",
