...
from Backend.Benchmark.evaluation_protocols.pipeline.consumption import load_weak_label_sources
from Backend.Benchmark.validity_lifecycle.contracts import ProtocolLifecycleInputs
from Backend.Benchmark.weak_labels.io import load_canonical_history
...
def load_protocol_lifecycle_inputs(evaluation_protocol_run_dir: Path) -> ProtocolLifecycleInputs:
    ...
    weak_labels_run_dir = Path(str(config["weak_labels_run_dir"])).resolve()
    ...
    weak_sources = load_weak_label_sources(weak_labels_run_dir)
    ...
    return ProtocolLifecycleInputs(
        ...
        point_labels_train=weak_sources.point_labels_train,
        point_labels_detailed=weak_sources.point_labels_detailed,
        point_evidence_flags=weak_sources.point_evidence_flags,
        v2_same_y_labels=weak_sources.v2_same_y_labels,
        v2_temporal_evidence_3h=weak_sources.v2_temporal_evidence_3h,
        v2_temporal_evidence_8h=weak_sources.v2_temporal_evidence_8h,
        v2_temporal_labels_3h=weak_sources.v2_temporal_labels_3h,
        v2_temporal_labels_8h=weak_sources.v2_temporal_labels_8h,
