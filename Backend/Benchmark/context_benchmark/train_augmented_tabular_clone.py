...
from Backend.Benchmark.context_benchmark.src.config.settings import CONTEXT_BENCHMARK_ROOT, ContextBenchmarkConfig
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Reuse an existing built context_benchmark clone dataset run. Skips dataset build when provided.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to synthetic_benchmark_gap_aware.csv. Overrides simulator-clone-run-dir when provided.",
    )
    parser.add_argument(
        "--label-scheme",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional output root for the dataset build stage. Defaults to context_benchmark/artifacts/builds/<label_scheme>/clone.",
    )
    parser.add_argument(
        "--train-output-root",
        type=Path,
        default=None,
        help="Optional output root for the training stage. Defaults to context_benchmark/artifacts/training/<label_scheme>/clone.",
    )
...
def _default_clone_build_root(label_scheme: str) -> Path:
    return (CONTEXT_BENCHMARK_ROOT / "artifacts" / "builds" / label_scheme / "clone").resolve()


def _default_clone_training_root(label_scheme: str) -> Path:
    return (CONTEXT_BENCHMARK_ROOT / "artifacts" / "training" / label_scheme / "clone").resolve()
...
def _resolve_synthetic_gap_aware_csv(args: argparse.Namespace) -> Path:
    ...
    synthetic_csv = (run_dir / "synthetic_benchmark_gap_aware.csv").resolve()
