...
DEFAULT_REAL_EVENT_CSV = DEFAULT_DATASET_ROOT / "benchmark_input_labeled.csv"
...
class ContextBenchmarkConfig:
    benchmark_family: str = "context_benchmark"
    ...
    def resolve_defaults(self) -> None:
        ...
        if self.output_root is None:
            self.output_root = default_context_build_root(CONTEXT_BENCHMARK_ROOT, self.label_scheme)
        else:
            self.output_root = self.output_root.resolve()
        latest_run = _latest_simulator_run_dir()
        if self.synthetic_gap_aware_csv is None:
            self.synthetic_gap_aware_csv = (latest_run / "synthetic_benchmark_gap_aware.csv").resolve()
        else:
    ...
    def validate(self) -> None:
        ...
        if not self.real_event_csv.exists():
            raise FileNotFoundError(
                ...
                "context_benchmark requires a labeled real-data artifact with big_label plus telemetry-gap/provenance fields "
                "from benchmark_dataset real_labeling. Rebuild "
                "Backend/Benchmark/benchmark_dataset/dataset/benchmark_input_labeled.csv first, or pass "
