...
def _candidate_build_roots(preferred_output_root: Path) -> list[Path]:
    ...
    candidates = [
        scheme,
        (CONTEXT_BENCHMARK_ROOT / "artifacts" / "builds" / "four_class" / "augmented").resolve(),
        (CONTEXT_BENCHMARK_ROOT / "outputs_option2_4class").resolve(),
        (CONTEXT_BENCHMARK_ROOT / "outputs").resolve(),
    ]
    unique: list[Path] = []
    seen: set[Path] = set()
...
class ContextTrainConfig:
    ...
    def resolve_defaults(self) -> None:
        ...
        if self.output_root is None:
            self.output_root = default_context_training_root(CONTEXT_BENCHMARK_ROOT, self.label_scheme)
        else:
            self.output_root = self.output_root.resolve()
        if self.build_run_dir is None:
            preferred_build_root = default_context_build_root(CONTEXT_BENCHMARK_ROOT, self.label_scheme)
