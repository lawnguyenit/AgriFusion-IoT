...
from Backend.Benchmark.context_benchmark.src.data.training_io import load_build_manifest, load_tabular_bundle
from Backend.Benchmark.context_benchmark.src.pipeline.scientific_artifact_pipeline import (
...
def run_training_pipeline(config: ContextTrainConfig) -> dict[str, object]:
    ...
    print(f"[context_benchmark] label_scheme={config.label_scheme} output_dir={output_dir}")

    for experiment_index, experiment_name in enumerate(config.experiment_names, start=1):
        print(f"[context_benchmark] experiment {experiment_index}/{len(config.experiment_names)} -> {experiment_name}")
    ...
    print(
        f"[context_benchmark] best={best_row['experiment_name']}/{best_row['model_name']} "
