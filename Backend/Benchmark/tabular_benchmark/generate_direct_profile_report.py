...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Word-friendly profile report for tabular benchmark raw features.")
    parser.add_argument(
        "--run-dir",
        type=Path,
        default=None,
        help="Path to a tabular benchmark run directory. Defaults to the latest run under tabular_benchmark/artifacts.",
    )
    parser.add_argument(
        "--experiment",
        type=str,
...
def discover_latest_run_dir(output_root: Path) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(f"No tabular benchmark runs found under {output_root}")
