...
from Backend.Benchmark.tabular_benchmark.src.pipeline.train_pipeline import run_training_pipeline


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the unified 3-model suite from a prepared tabular benchmark build run."
    )
    parser.add_argument(
        "--build-run-dir",
        type=Path,
        default=None,
        help="Prepared tabular benchmark build run directory. Defaults to the latest run for the selected label lane.",
    )
    parser.add_argument(
        "--label-mode",
