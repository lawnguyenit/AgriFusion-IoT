...
def summarize_protocol_classification(
    ...
    environment_set = sorted({str(environment_id) for environment_id in (environment_ids or []) if environment_id is not None})
    support_restricted_environment = environment_set == ["E3"]
    ...
    for index, class_name in enumerate(class_names):
        ...
        class_rows[class_name] = {
            "precision": float(precision[index]) if estimable else math.nan,
            "recall": float(recall[index]) if estimable else math.nan,
            "f1_score": float(f1[index]) if estimable else math.nan,
    ...
    return {
        ...
        "environment_ids": environment_set,
        "metric_contract_id": "observed_support_metrics" if support_restricted_environment else "macro_f1_3class",
        "cross_environment_score_delta_allowed": not support_restricted_environment,
