...
from Backend.Benchmark.benchmark_dataset.multi_window_features.src.pipeline import (
    MultiWindowBuildResult,
    build_multi_window_experiments,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build multi-window benchmark feature CSVs from the aligned benchmark dataset."
    )
    parser.add_argument(
        "--input-csv",
        type=Path,
...
def _print_result(result: MultiWindowBuildResult) -> None:
...
def main() -> None:
    ...
    results = build_multi_window_experiments(
    ...
    report_payload = build_stage_report(
        ...
        results=results,
        notes=[
            "Multi-window features build train-facing exports from the aligned or labeled benchmark dataset source.",
            "This stage does not generate labels and only retains big_label when it is already present.",
        ],
    )
    report_path = results[0].output_csv.parent / "multi_window_feature_build_report.json"
    write_report(report_path, report_payload)
    print("Multi-window feature datasets complete")
