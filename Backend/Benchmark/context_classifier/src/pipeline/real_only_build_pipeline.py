...
from Backend.Benchmark.context_classifier.src.data.canonical_builder import write_label_summary
...
from Backend.Benchmark.context_classifier.src.data.training_io import load_build_manifest
...
def _default_real_only_output_root(source_build_run_dir: Path) -> Path:
    context_classifier_root = source_build_run_dir.parents[4] if source_build_run_dir.parent.name in {"augmented", "real_only"} else source_build_run_dir.parents[2]
    ...
    return (context_classifier_root / "artifacts" / "builds" / source_label_scheme / "real_only").resolve()
...
def run_real_only_build_pipeline(
    *,
    ...
    manifest = {
        "benchmark_family": source_manifest.get("benchmark_family", "context_classifier"),
