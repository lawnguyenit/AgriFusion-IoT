...
from Backend.Benchmark.ft_transformer_benchmark.src.reports.report_pipeline import (
    TrainingRunSpec,
    run_report_pipeline,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate academic charts and summary tables for FT-Transformer benchmark runs."
    )
    parser.add_argument(
        "--run-dir",
        type=Path,
        action="append",
        required=True,
        help="Training run directory under ft_transformer_benchmark/outputs.",
    )
    parser.add_argument(
        "--run-label",
        action="append",
