...
from Backend.Benchmark.common.paths import FT_TRANSFORMER_BENCHMARK_ROOT
from Backend.Benchmark.ft_transformer_benchmark.src.scientific_artifacts import TOP1_CALIBRATION_LABEL
...
def _ensure_report_root() -> Path:
    return (FT_TRANSFORMER_BENCHMARK_ROOT / "reports").resolve()
...
def _write_markdown_summary(
    *,
    ...
    lines: list[str] = []
    lines.append("# FT-Transformer Benchmark Report")
    lines.append("")
    ...
    if scientific_metrics is not None and not scientific_metrics.empty:
        ...
        lines.append("")
        lines.append("Cac chi so duoi day duoc rut ra tu artifact FT-Transformer luu trong luc train lai.")
        lines.append("")
...
def run_report_pipeline(*, run_specs: list[TrainingRunSpec]) -> dict[str, object]:
    ...
    run_id, output_dir = create_run_directory(report_root, prefix="ft_report")
    ...
    _metric_bar_plot(
        ...
        title="So sanh test_macro_f1 giua cac arm FT-Transformer benchmark",
        output_path=output_dir / "chart_test_macro_f1.png",
    )
    _metric_bar_plot(
        summary_metrics,
        metric_column="test_accuracy",
        title="So sanh test_accuracy giua cac arm FT-Transformer benchmark",
    ...
    manifest = {
        "report_kind": "ft_transformer_benchmark_report",
        "run_id": run_id,
