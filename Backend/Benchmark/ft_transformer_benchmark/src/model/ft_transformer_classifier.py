...
class FTTransformerBlock(nn.Module):
    ...
    def forward(self, inputs: Tensor) -> tuple[Tensor, Tensor]:
        ...
        attention_out, attention_map = self.attention(
            normalized,
            normalized,
            normalized,
            need_weights=True,
...
class FTTransformerClassifier(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, config: FTTransformerClassifierConfig) -> None:
        super().__init__()
        self.tokenizer = NumericalFeatureTokenizer(num_features=input_dim, token_dim=config.token_dim)
        self.input_projection = (
            nn.Identity()
            if config.token_dim == config.model_dim
            else nn.Linear(config.token_dim, config.model_dim)
        )
    ...
    def forward(self, inputs: Tensor) -> tuple[Tensor, dict[str, float]]:
        ...
        tokens = self.dropout(tokens + self.feature_embeddings[:, : tokens.shape[1], :])
        ...
        for block in self.blocks:
            ...
            entropy = -(attention_prob * attention_prob.log()).sum(dim=-1).mean()
            attention_entropy_values.append(entropy)
        cls_representation = tokens[:, 0, :]
        logits = self.head(cls_representation)
        diagnostics = {
            "attention_entropy": float(torch.stack(attention_entropy_values).mean().detach().cpu().item())
