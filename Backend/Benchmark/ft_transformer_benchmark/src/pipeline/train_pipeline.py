...
from Backend.Benchmark.ft_transformer_benchmark.src.scientific_artifacts import (
    probabilities_from_logits,
    write_ft_scientific_artifacts,
)
...
def run_ft_transformer_pipeline(config: FTTransformerBenchmarkConfig) -> dict[str, object]:
    config.validate()
    ...
    print(f"[ft_transformer_benchmark] run_id={run_id} output_dir={output_dir}")

    for experiment_index, experiment_name in enumerate(config.experiments, start=1):
        print(f"[ft_transformer_benchmark] experiment {experiment_index}/{len(config.experiments)} -> {experiment_name}")
    ...
    print(
        f"[ft_transformer_benchmark] best={best_row['experiment_name']}/{best_row['model_name']} "
...
def _run_single_experiment(
    *,
    ...
    if "ft_transformer_classifier" in config.model_names:
        print(f"[ft_transformer:{experiment_name}] training ft_transformer_classifier")
        ft_result = train_ft_transformer_classifier(
            ...
            progress_label=f"ft:{experiment_name}:transformer",
        )
        device = next(ft_result.model.parameters()).device
        with torch.no_grad():
            train_logits, _ = ft_result.model(torch.tensor(train_scaled, dtype=torch.float32, device=device))
            validation_logits, _ = ft_result.model(torch.tensor(validation_scaled, dtype=torch.float32, device=device))
            test_logits, _ = ft_result.model(torch.tensor(test_scaled, dtype=torch.float32, device=device))
        ...
        test_metrics = summarize_classification(
            test_labels,
            test_logits.argmax(dim=1).cpu().numpy(),
            label_policy.class_names,
        )
        train_metrics = summarize_classification(
            train_labels,
            train_predictions,
            label_policy.class_names,
        )
        ...
        train_probabilities = probabilities_from_logits(train_logits.detach().cpu().numpy())
        validation_probabilities = probabilities_from_logits(validation_logits.detach().cpu().numpy())
        test_probabilities = probabilities_from_logits(test_logits.detach().cpu().numpy())
        ft_scientific_artifacts = write_ft_scientific_artifacts(
            ...
            training_config=FTTransformerClassifierConfig(
                batch_size=config.ft_batch_size,
                max_epochs=config.ft_max_epochs,
                patience=config.ft_patience,
                learning_rate=config.ft_learning_rate,
            ...
            training_metadata={
                ...
                "best_validation_loss": ft_result.best_validation_loss,
                "total_epochs_ran": ft_result.total_epochs_ran,
                "stop_reason": ft_result.stop_reason,
                "total_train_seconds": ft_result.total_train_seconds,
                "device": ft_result.device,
                "device_name": ft_result.device_name,
                "class_weights": ft_result.class_weights,
        ...
        scientific_artifacts_by_model["ft_transformer_classifier"] = ft_scientific_artifacts
        scientific_summaries_by_model["ft_transformer_classifier"] = ft_scientific_artifacts["split_summaries"]
        model_results.append(
            ModelResult(
                ...
                metrics={
                    ...
                    "test": test_metrics,
                    "history": ft_result.history,
                    "scientific_split_summaries": ft_scientific_artifacts["split_summaries"],
                },
                notes=(
                    f"Best epoch {ft_result.best_epoch}; "
                    f"scientific_artifacts={ft_scientific_artifacts['manifest_path']}"
                ),
            )
        )

    sklearn_model_names = [name for name in config.model_names if name != "ft_transformer_classifier"]
    sklearn_results = train_model_suite(
        ...
        progress_prefix=f"ft:{experiment_name}:sklearn",
    )
