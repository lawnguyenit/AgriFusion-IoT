...
from Backend.Benchmark.direct_benchmark.src.data.training_io import (
...
def run_training_pipeline(config: DirectBenchmarkTrainConfig) -> dict[str, object]:
    ...
    print(
        f"[direct_benchmark:train] label_mode={config.label_mode} run_id={run_id} output_dir={output_dir}"
    )

    for experiment_index, experiment_name in enumerate(config.experiments, start=1):
        print(
            f"[direct_benchmark:train] experiment {experiment_index}/{len(config.experiments)} -> {experiment_name}"
        )
    ...
    print(
        f"[direct_benchmark:train] best={best_row['experiment_name']}/{best_row['model_name']} "
...
def _run_single_experiment(
    *,
    ...
    if "tabnet_classifier" in config.model_names:
        print(f"[direct_benchmark:{config.label_mode}:{experiment_name}] training tabnet_classifier")
    ...
    if "ft_transformer_classifier" in config.model_names:
        print(f"[direct_benchmark:{config.label_mode}:{experiment_name}] training ft_transformer_classifier")
