...
def run_build_pipeline(config: DirectBenchmarkBuildConfig) -> dict[str, object]:
    ...
    print(
        f"[direct_benchmark:build] label_mode={config.label_mode} run_id={run_id} output_dir={output_dir}"
    )
    for experiment_index, experiment_name in enumerate(config.experiments, start=1):
        print(
            f"[direct_benchmark:build] experiment {experiment_index}/{len(config.experiments)} -> {experiment_name}"
        )
