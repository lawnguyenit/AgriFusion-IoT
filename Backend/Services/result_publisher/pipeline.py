...
try:
    ...
    from Services.result_publisher.ft_runtime import (
        ContextFTRuntimeDiagnosisModel,
        ContextFTModelArtifact,
        build_runtime_feature_row,
        discover_best_ft_context_artifact,
        display_label_for_context,
    )
except ModuleNotFoundError:
    from ...Config.runtime import BackendSettings
    from .ft_runtime import (
        ContextFTRuntimeDiagnosisModel,
        ContextFTModelArtifact,
        build_runtime_feature_row,
        discover_best_ft_context_artifact,
...
class RuntimeDiagnosisModel:
    def __init__(self, artifact: BenchmarkModelArtifact):
        ...
        self.imputer, self.scaler = self._build_runtime_transformers()
    ...
    def predict(self, feature_row: dict[str, float | None]) -> dict[str, Any]:
        ...
        return {
            ...
            "abnormalProbability": float(probability_map.get("abnormal", 0.0)),
...
class ResultPublisherPipeline:
    ...
    def run(self, *, mode: str = "snapshot", dry_run: bool = False) -> ResultPublishRunResult:
        ...
        diagnosis_payload, model_artifact = self._build_diagnosis(bundle)
        ...
        if requested_mode == "append" and previous_state is None:
        ...
        history_counts = {group: len(records) for group, records in bundle["histories"].items()}
        history_last_ts = {
    ...
    def _load_layer1_bundle(self) -> dict[str, Any]:
        ...
        raw_latest = {
            "sht30": _load_json(layer1_root / "sht30" / "latest.json"),
            "npk": _load_json(layer1_root / "npk" / "latest.json"),
        }
        histories = {
        ...
        feature_rows = self._build_feature_rows(histories["air"], histories["soil"])
    ...
    def _build_diagnosis(
        ...
        ft_artifact = self._discover_best_runtime_context_ft()
        if ft_artifact is not None:
            runtime_feature_row = build_runtime_feature_row(self.settings, ft_artifact)
            if runtime_feature_row is None:
                return (
                    {
                        "status": "pending_features",
                        "label": "unknown",
                        "labelId": -1,
                        "displayLabel": "Chua du dac trung de phan loai",
            ...
            try:
                model = ContextFTRuntimeDiagnosisModel(ft_artifact)
                prediction = model.predict(runtime_feature_row)
                ...
                return (
                    {
                        ...
                        "label": "unknown",
                        "labelId": -1,
                        "displayLabel": "Loi mo hinh FT-Transformer",
                        "abnormalProbability": None,
                        "ts": runtime_feature_row.get("timestamp"),
                        "error": repr(exc),
                    },
                    None,
                )

            abnormal_probability = float(prediction["abnormalProbability"])
            ...
            return (
                {
                    ...
                    "model": {
                        ...
                        "labelScheme": ft_artifact.label_scheme,
                        "experimentName": ft_artifact.experiment_name,
                        "artifactPath": str(ft_artifact.artifact_path),
                        "validationMacroF1": ft_artifact.validation_macro_f1,
                        "testMacroF1": ft_artifact.test_macro_f1,
                    },
                },
                None,
            )
        ...
        if latest_feature_row is None:
            return None, None
        model_artifact = self._discover_best_runtime_xgboost()
        if model_artifact is None:
            return (
                {
        ...
        return (
            {
                "status": "ready",
                "label": label,
                "displayLabel": display_label_for_context("packet_loss_outage" if label == "abnormal" else "normal_context"),
                "labelId": prediction["labelId"],
                "abnormalProbability": abnormal_probability,
                "probabilities": prediction["probabilities"],
                "severity": severity,
    ...
    def _publish_append(self, *, payload: dict[str, Any], previous_state: dict[str, Any] | None) -> bool:
        if previous_state is None:
            return self.firebase_service.set_data(self.result_path, payload)
    ...
    def _discover_best_runtime_xgboost(self) -> BenchmarkModelArtifact | None:
        direct_root = self.paths.benchmark_root / "direct_benchmark" / "outputs"
        ...
        for report_path in direct_root.rglob("training_report.json"):
            ...
            for model_payload in report.get("models", []):
                ...
                artifact_path = Path(model_payload["artifact_path"])
                candidate = BenchmarkModelArtifact(
                    report_path=report_path,
                    experiment_dir=experiment_dir,
                    experiment_name=str(report.get("experiment_name", experiment_dir.name)),
                    source_kind=str(report.get("source_kind", report.get("experiment_name", experiment_dir.name))),
                ...
                if not (
                    ...
                    and candidate.scaler_path.exists()
                    and candidate.imputer_path.exists()
                    and candidate.label_policy_path.exists()
                ):
                    continue
                if best_candidate is None or candidate.validation_macro_f1 > best_candidate.validation_macro_f1:
                    best_candidate = candidate
        return best_candidate

    def _discover_best_runtime_context_ft(self) -> ContextFTModelArtifact | None:
        return discover_best_ft_context_artifact(self.paths.benchmark_root)
    ...
    def _build_feature_rows(
        ...
        common_timestamps = sorted(set(air_history).intersection(soil_history))
    ...
    def _load_latest_group_points(self) -> dict[str, dict[str, Any]]:
        layer1_root = self.settings.layer1_root
        return {
            "air": self._record_to_group_point(_load_json(layer1_root / "sht30" / "latest.json"), group_name="air"),
            "soil": self._record_to_group_point(_load_json(layer1_root / "npk" / "latest.json"), group_name="soil"),
            "npk": self._record_to_group_point(_load_json(layer1_root / "npk" / "latest.json"), group_name="npk"),
            "weather": {},
        }
