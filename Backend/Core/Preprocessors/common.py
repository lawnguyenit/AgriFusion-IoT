...
def build_window_stats(
    ...
    for hours in window_hours:
        ...
        subset = [
            ...
            if (ts := safe_int(record.get("timestamps", {}).get("ts_server"))) is not None
            and ts >= window_start
        ]
        timestamps = [
            safe_int(record.get("timestamps", {}).get("ts_server"))
            for record in subset
            if safe_int(record.get("timestamps", {}).get("ts_server")) is not None
        ]
        ...
        for metric_key in metric_keys:
            ...
            metric_timestamps = [
                ...
                and (ts := safe_int(record.get("timestamps", {}).get("ts_server"))) is not None
            ]
