...
def series_stats(values: Sequence[float], timestamps: Sequence[int]) -> dict[str, Any]:
    ...
    trend_per_hour = round(delta / hours, 4) if hours > 0 else None
    ...
    for hours in window_hours:
        ...
        timestamped_records = sorted(
            (
                ...
                if (ts := resolve_window_ts(record)) is not None and selection_start <= ts <= observed_ts
            ),
        ...
        regular_gap_count = sum(1 for gap in gaps if min_regular_gap_sec <= gap <= max_regular_gap_sec)
        ...
        gap_continuity_ratio = (
            regular_gap_count / len(gaps) if gaps else (1.0 if usable_for_trend else 0.0)
        )
        ...
        if gaps:
            ...
            window_payload["coverage_hours"] = round((timestamps[-1] - timestamps[0]) / HOUR_SECONDS, 3)
